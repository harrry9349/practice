...
//かし、その頑丈で大きな建物を完成させるには、大きな太い柱が必要です。町（町２）にそんな大きな柱はありません。
//そこで、大工は遠くの山里（町４）まで大きな柱を調達しに行くことになりました（大工は町から里山に行って、町に戻ってくる必要はあります）。

//大工の褒美は、殿様から受け取ったお金から柱の値段と交通費を差し引いた余りです。
...
func main() {
	...
	result := prize - tree - BellmanFord(input, start, goal)

	// 帰り６→１についてベルマンフォード法を実行する
	result = result - BellmanFord(input, goal, start)
